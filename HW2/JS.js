...
function loginOK(id){
    ...
    var todayDate = new Date(); // 오늘 날짜 받아오기
    // var todayDate = new Date(2020,9,3); // 임의 날짜 선택하기
...
function setNewToDo(title, ID){
    ...
    newToDoDiv.addEventListener("click",function(){
        ...
        add_date.setAttribute('min',null);
        add_date.setAttribute('max',null);

        sessionStorage.setItem("clickedBlock",ID);
    });
...
cancleBtn.addEventListener("click",function(){
    addBackDiv.style.display="none";
    sessionStorage.removeItem("clickedBlock");

});
submitBtn.addEventListener("click",function(){
    var getEventId = sessionStorage.getItem("clickedBlock");
    $.post("update.php",
        {
            date : $("#add_date").val(),
            time : $("#add_time").val(),
        ...
        function(data){
            ...
            var updated = returnData["date"].split('-');
            var updateDate = new Date(updated[0], updated[1]-1, updated[2]);
            checkToday(updateDate);
